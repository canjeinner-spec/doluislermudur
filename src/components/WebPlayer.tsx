...
type Props = {
  /** Provider content URL (the user is already signed in via persisted cookies). */
  uri: string;
  userAgent?: string;
...
/**
 * The controller injected into the provider page. It locates the largest
 * <video> element (works across Netflix / YouTube / Vimeo / direct sites),
 * reports its state back to React Native, and applies play/pause/seek commands.
 * DRM stays intact — playback happens in the provider's own player with the
 * user's own account; we only orchestrate the existing element.
 */
const CONTROLLER = `
(function () {
  if (window.__astera) { return; }
  window.__astera = true;
  ...
  window.__asteraApply = function (cmd) {
    var v = findVideo();
    if (!v) return;
    try {
      ...
      else if (cmd.type === 'mute') { v.muted = !!cmd.value; }
    } catch (e) {}
    setTimeout(report, 60);
  };
  ['play', 'pause', 'seeked', 'ended', 'loadedmetadata', 'ratechange'].forEach(function (ev) {
...
export function WebPlayer({ uri, userAgent, onControl }: Props) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Control overlay — tap toggles visibility. */}
      <PressableScale
        onPress={() => (controlsVisible ? revealControls() : revealControls())}
        activeScale={1}
        activeOpacity={1}
      ...
      >
        ...
        <View style={styles.bottom} pointerEvents="box-none">
          <View style={styles.timeRow}>
            ...
            {!hasVideo && <Text style={styles.hint}>Sitede oynatmayı başlatın</Text>}
